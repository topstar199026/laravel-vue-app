  ...
  <v-card class="max-h-content-container app-email h-full position-relative overflow-hidden text-sm">
    ...
    <div class="email-content-area h-full position-relative" :style="contentStyles">
      ...
      <div class="d-flex align-center px-4 py-2">
        ...
        <v-spacer></v-spacer>
        <v-btn icon small class="me-2" @click="fetchEmails">
          <v-icon size="21">
            {{ icons.mdiReload }}
          </v-icon>
        </v-btn>
...
// Local Componnets
import EmailLeftSidebarContent from './EmailLeftSidebarContent.vue'
...
export default {
  ...
  setup() {
    ...
    watch([searchQuery, routeParams], () => fetchEmails())
    ...
    // ? You can also use isEmailDetailsOpen (instead of `emailViewData`) but it will trigger execution twice in this case
    // eslint-disable-next-line no-use-before-define
...
<style lang="scss">
@import '~@resources/sass/preset/mixins';
