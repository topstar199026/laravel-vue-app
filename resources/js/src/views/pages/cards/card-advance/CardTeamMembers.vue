<template>
  <v-card>
    ...
    <v-simple-table class="team-table">
      ...
      <tbody>
        <tr v-for="team in teamMembers" :key="team.chipText">
          <td class="d-flex">
            <v-avatar size="38">
          ...
          <td>
            <v-chip small color="primary" :class="`v-chip-light-bg ${team.chipColor}--text font-weight-semibold`">
              {{ team.chipText }}
            </v-chip>
          </td>
          <td>
            <p class="font-weight-semibold mb-0">
              <span class="primary--text">{{ team.task.split('/')[0] }}</span
              >/<span class="text--primary">{{ team.task.split('/')[1] }}</span>
            </p>
          </td>
